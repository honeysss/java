...
//	通过部门id查询员工
	public ArrayList<Employee> selectUserByDeptId(int deptId) throws SQLException {
		ArrayList<Employee> employeeList = new ArrayList<Employee>();
...
//	通过员工名和id查询用户
	public Employee selectByIdAndName(int empId, String empName) throws SQLException {
		Employee employee = null;
...
//	查询最新的员工id
	public int getNewEmpId() throws SQLException {
		int empId = 0;
		String sql = "select empId from employee ORDER BY empId desc limit 0,1";
		PreparedStatement ps = (PreparedStatement) getConnection().prepareStatement(sql);
		ResultSet rs = ps.executeQuery();
		while(rs.next()) {
			empId = rs.getInt("empId");
	...
	//通过员工id找到员工部门id
		public int selectDeptIdById(int empId) throws SQLException {
			int deptId = 0;
			String sql = "select empDeptId from employee where empId = ?";
			PreparedStatement ps = (PreparedStatement) getConnection().prepareStatement(sql);
			ps.setInt(1, empId);
			ResultSet rs = ps.executeQuery();
			while(rs.next()) {
...
//	通过员工id把员工的部门id设为-1
		public void updateDeptId(int empId) throws SQLException {
			String sql = "update employee set empDeptId = -1 where empId = ?";
			PreparedStatement ps = (PreparedStatement) getConnection().prepareStatement(sql);
			ps.setInt(1, empId);
			ps.executeUpdate();
			getConnection().close();
